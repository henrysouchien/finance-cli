...
12. [New User Quick Start](#12-new-user-quick-start) — First-session setup for new users with 1 week to 3 months of data
13. [Error Triage](#error-triage) — Review captured runtime errors and drive fixes
14. [Loan Management](#loan-management) — Track informal debts, record payments, project payoff
15. [AI-Driven Onboarding](#ai-driven-onboarding) — Conversational first-run setup in chat
...
### Quick Snapshot (for < 3 months of data)

If the user has less than 3 months of transaction history, a full gap analysis isn't reliable yet. Run a simplified snapshot instead:

1. `financial_summary` — current net worth, this month's cash flow
2. `balance_net_worth` — asset/liability breakdown
3. `debt_dashboard` — debt overview (if any)
4. `subs_total` — subscription burn rate

Present a simplified view: "Here's where you stand today. Once you have 3 months of data, we'll run the full gap analysis for trend detection, seasonal patterns, and actionable recommendations."

Then redirect to New User Quick Start (#12) if basic infrastructure (budgets, goals) isn't set up yet.
...
- Clear micro-balances — pay off cards under $50-100 to simplify
...
**Include a Milestones section** — specific, time-bound targets (e.g., "Month 1: gap reduced to under $5,000").
...
- **The debt trap was the critical finding**: $1,786/mo interest > $1,415/mo minimums = debt grows $371/mo even at perfect payments. This reframed the entire conversation from "how to pay off debt" to "how to stop losing $371/mo before anything else."
- **Subscription audit found different savings than expected**: Stale subs were a small win ($200/mo cancelled), but the real insight was that $675/mo goes to AI dev tooling (Claude + OpenAI) — a legitimate business expense, not waste.

---

## Error Triage

**Goal:** Work a captured runtime error from discovery to resolution without digging through raw log files.

**When to run:** A user reports a failure, alerts indicate a new or recurring error, or the agent needs to inspect open backend issues before making a code change.

### Steps

1. Run `error_list` or `python3 -m finance_cli error list --days 30 --format json` to filter open errors by severity, source, or recency.
2. Run `error_show` or `python3 -m finance_cli error show <error_id> --format json` for the selected error and read the redacted traceback, context, and occurrence timeline.
3. Grep the codebase for the reported module, function, endpoint, or error message to find the implementation path.
4. Propose the fix, explain the suspected root cause, and implement or stage the code change.
5. Run `error_update` or `python3 -m finance_cli error update <error_id> --status resolved --resolution "..." --format json` to move the record to `investigating`, `resolved`, or `wontfix`, adding the resolution note when known.

### Notes

- Prioritize `critical` errors first, then high-frequency `error` rows with recent alerts.
- Use the fingerprint and endpoint to avoid treating duplicate occurrences as separate bugs.
- When the fix lands, include the commit hash or concrete remediation note in `error_update`.
- For agent-reported issues rather than runtime exceptions, use `issue_list` / `issue_update` or the direct `python3 -m finance_cli issue list|update ...` equivalents.
- **Income was identified as the primary lever**: With $3,750/mo rent (non-negotiable short-term), expense cuts max out around $1,500/mo. Closing a $6,700 gap requires $3,000+/mo in new income.
...
|-----------|--------|-----------|---------|
| Emergency fund | `liquid_cash` | up | $25,000 in liquid savings |
| Debt freedom | `total_debt` | down | $0 total debt |
| Net worth milestone | `net_worth` | up | $100,000 net worth |
| Investment target | `investments` | up | $200,000 in investments |
...
Options:
- `--method simplified` — IRS simplified method ($5/sqft, max 300 sqft = $1,500 deduction)
...
Shows per-contractor payment totals. Flags contractors paid $600+ (1099-NEC threshold).
...
- **1099 threshold approaching** → Proactively flag when a contractor is nearing $600 to ensure the user has their TIN on file
...
3. **`budget suggest`** — cut suggestions roll up to parent groups so recommendations are actionable at the right level (e.g., "cut Food & Drink by $36" instead of three separate line items).
...
1. **Inflated amounts from plan fees** — Amex "Pay It Plan It" creates `PLAN FEE - <vendor>` transactions at $1-2 each. The detector sees these as the subscription charge instead of (or in addition to) the real charge, inflating the detected amount and distorting frequency.
...
4. Plan fees ($1-2 Amex installment fees) inflate detected amounts
...
From the March 2026 audit (baseline $1,161/mo → cleaned $1,189/mo):
...
- **Apple.com/Bill corrected** — was tracked as 1 Business sub at $10.88 biweekly; actually 3 Personal recurring charges totaling $42.19/mo
- **Cursor amount was wrong** — showed $472/mo but real charges were $217.75/mo (plan fees inflated the amount)
...
### Setting Budgets Without a Gap Analysis (Starter Path)

For users who haven't done a gap analysis yet (typically < 3 months of data):

1. Run `spending_trends` for whatever data exists (even 1 month)
2. Use current spending levels as the baseline — don't try to reduce yet
3. Set budgets at approximately current levels for the top 3-5 discretionary categories
4. Frame as maintenance budgets: "These track your current spending so you'll notice if anything spikes. After a full gap analysis, we can set reduction targets."

This is the approach used in New User Quick Start (#12). The full workflow below applies after a gap analysis has identified specific reduction opportunities.
...
|---|---|---|---|---|
| Dining | $783 | $800 | $400 | $383 |
| Shopping | $287 | $400 | $150 | $137 |
| ... | ... | ... | ... | ... |
...
**Agent decisions:**
- Total savings from discretionary budget cuts should align with the action plan target (e.g., ~$1,000/mo)
- If the user pushes back on a target, negotiate — a $500 Dining budget is better than no budget even if $400 was the plan
...
- How the savings feed into debt payoff (e.g., "$965/mo in budget savings → applied to highest-APR card")
...
- **Budget targets came from the action plan** — not arbitrary. Each target was calculated from the gap analysis: "we need $1,000/mo in discretionary savings, here's how it splits across categories." This linkage is critical — budgets without a reason behind them don't stick.
...
OVER BUDGET:
  Dining [Personal]: $420 spent / $400 budget (105%) — $20 over

AT RISK (projected to exceed):
  Shopping [Personal]: $95 spent / $150 budget — on pace for $196 ($46 over)

WARNING (>80% projected):
  Coffee [Personal]: $25 spent / $40 budget — on pace for $52 ($12 over)
...
- **AT RISK**: There's still time to course-correct. Show the user:
  ...
  - `remaining_daily_budget` field tells them exactly: "$3.68/day remaining for Shopping"
...
- If discretionary budgets (Dining, Shopping, Entertainment, Coffee, Travel) are consistently AT RISK or OVER, the $965/mo savings target won't be met
...
- If **at risk** (projected to exceed): Call it out with the daily remaining: "Shopping is on pace to exceed budget by $47. You have $3.68/day remaining to stay on track."
- If **over budget**: Lead with it: "Dining is already $20 over the $400 budget with 16 days left in the month."
...
- **The `remaining_daily_budget` field is the most actionable number** — it tells the user exactly how much they can spend per day to stay on budget (e.g., "$13.11/day for Dining")
...
**Reference implementation:** See `docs/completed/CATEGORY_DATA_CLEANUP_PLAN.md` for a completed example of this workflow.
...
1. **Run cross-format dedup** — catch duplicates across import paths (CSV vs PDF vs Plaid), including ±2 day date offsets:
   ```
...
Based on account state, recommend the appropriate next workflow:

- **First import (no budgets/goals set):** → Run New User Quick Start (#12) to set up categories, subscriptions, budgets, and a first goal
- **Budgets and goals already exist:** → Run Monthly Review (#2) to incorporate the new data
- **3+ months of data, no gap analysis done yet:** → Run Gap Analysis (#1) for a comprehensive diagnostic
...
**Goal:** Help users track informal debts (family loans, personal IOUs), record payments, and project payoff timelines. Manual loans integrate into debt dashboard, net worth, obligations, and financial summary.

**When to run:** When a user mentions owing money informally ("I borrowed from my parents", "I owe a friend $5K"), wants to track repayments, or needs to see how informal debts affect their financial picture.

**Prerequisites:**
- None — loan tracking is standalone, no external integrations needed

### Phase 1: Record the Loan

1. Gather loan details conversationally:
   - Who is the creditor? (e.g., "Mom", "Friend Bob")
   - How much was borrowed? (total, or multiple tranches over time)
   - When did it start?
   - Any agreed payment terms? (monthly amount, due day)
   - Any interest? (most family loans are 0%)

2. Create the loan:
   ```
   loan_add(creditor="Mom", amount=5000, start_date="2025-06-01",
            monthly_payment=500, due_day=1, description="House down payment help")
   ```

3. If borrowed in multiple tranches, add each disbursement:
   ```
   loan_disburse(loan_id="<id>", amount=2000, date="2025-09-15", notes="Additional help")
   ```

### Phase 2: Show Impact on Financial Picture

After creating the loan, show how it affects the user's finances:

1. **Debt dashboard** — `debt_dashboard()` — loan appears with "Loan: Mom" label
2. **Net worth** — `balance_net_worth()` — loan is subtracted from net worth
3. **Obligations** — `liability_obligations()` — monthly payment appears in fixed costs
4. **Payoff timeline** — `loan_schedule(loan_id="<id>")` — projected payoff date

Frame it: "This $5,000 loan reduces your net worth by that amount and adds $500/mo to your fixed obligations. At $500/mo with 0% interest, you'll be paid off in 10 months."

### Phase 3: Record Payments Over Time

When the user reports making a payment:

1. **Link to existing transaction** (preferred — maintains audit trail):
   ```
   # Find the payment transaction
   txn_search(query="Mom payment")
   # Link it
   loan_payment(loan_id="<id>", amount=500, transaction_id="<txn_id>")
   ```

2. **Manual recording** (if no matching transaction):
   ```
   loan_payment(loan_id="<id>", amount=500, date="2025-07-01", notes="Cash payment")
   ```

3. After recording, show updated status:
   ```
   loan_show(loan_id="<id>")  # balance, payment history, remaining
   ```

### Phase 4: Lifecycle Events

- **Adjust terms**: if payment amount or schedule changes:
  ```
  loan_adjust(loan_id="<id>", monthly_payment=300)
  ```

- **Close when paid off**: balance reaches 0 automatically, or close manually:
  ```
  loan_close(loan_id="<id>")
  ```

- **Debt forgiveness**: if creditor forgives the remaining balance:
  ```
  loan_close(loan_id="<id>", forgiven=True)
  ```

- **Borrow more**: reopens a closed loan if needed:
  ```
  loan_disburse(loan_id="<id>", amount=1000, notes="Additional help")
  ```

### Integration with Other Workflows

- **Debt Payoff Planning (#3)**: Manual loans automatically appear in `debt simulate` and `debt impact`. They rank lowest in avalanche strategy (0% APR).
- **Financial Gap Analysis (#1)**: Manual loans are included in net worth and obligations calculations.
- **Monthly Financial Review (#2)**: `loan_list()` shows outstanding balances; upcoming payments appear in `liability_upcoming()`.
- **Budget Monitoring (#9)**: Loan payments are part of fixed obligations in the summary dashboard.

## AI-Driven Onboarding

**Goal:** Take a new user from zero setup to a working dashboard in one guided chat session.

**When to run:** First authenticated web or Telegram session when the user has no imported data, or when resuming an incomplete onboarding.

### Flow Summary

Start by calling `onboarding_detect()` to determine whether onboarding should begin, resume, or be skipped.

1. **Data completeness**
   - Connect accounts with `plaid_link`, then `plaid_sync` and `plaid_balance_refresh`
   - Initialize setup with `setup_init`
   - Fill gaps with `ingest_csv` / `ingest_statement` when available
   - Run dedupe and first-pass categorization
   - Teach top uncategorized vendors
2. **Financial profile**
   - Ask natural questions about employment, income stability, priorities, debt, and emergency fund
   - Infer user type from transactions and income patterns
   - Persist profile in agent memory and onboarding state
3. **Assessment**
   - Present balances, cash flow, debt, liquidity, subscriptions, and business metrics as appropriate
   - Adapt depth to the amount of history available
4. **Setup and goals**
   - Propose realistic budgets
   - Create 1-2 starter goals
   - For mixed business/personal usage, add split rules with `rules_add_split`, then apply splits and tax setup
5. **Handoff**
   - Save onboarding notes and persistent profile
   - Show `financial_summary`
   - Explain the weekly and monthly follow-up cadence

### Resume Logic

- Call `onboarding_detect()` first to determine whether the user is new, mid-flow, or already complete.
- If `resume_checkpoint` is set, resume from that step rather than restarting from the top.
- If `is_onboarding_complete`, skip onboarding.
- Use explicit session markers such as `onboarding:phase1_complete` and `onboarding:complete`.

### Approval Pattern

- Low-risk infrastructure tools may run directly in onboarding mode.
- For high-value writes, summarize the batch in plain language, wait for the user's yes, then execute.

For the full conversation design, persistence model, and runtime requirements, see `docs/planning/SPEC_AI_ONBOARDING.md`.
