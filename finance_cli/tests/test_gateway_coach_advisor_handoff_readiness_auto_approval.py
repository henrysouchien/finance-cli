...
def test_coach_advisor_handoff_readiness_auto_approved_set_is_routine_only() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    assert tools.COACH_ADVISOR_HANDOFF_READINESS_AUTO_APPROVED == frozenset(
        {
            "agent_session_write",
    ...
    assert (
        ...
        not in tools.COACH_ADVISOR_HANDOFF_READINESS_AUTO_APPROVED
    )
    assert "goal_set" not in tools.COACH_ADVISOR_HANDOFF_READINESS_AUTO_APPROVED
    assert "budget_set" not in tools.COACH_ADVISOR_HANDOFF_READINESS_AUTO_APPROVED
    assert "notify_schedule" not in tools.COACH_ADVISOR_HANDOFF_READINESS_AUTO_APPROVED
    assert (
...
def test_advisor_handoff_read_tool_is_read_only_and_save_requires_approval() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    assert "coach_advisor_handoff_readiness_artifact_read" in tools.READ_ONLY_TOOLS
    assert (
        "coach_advisor_handoff_readiness_artifact_save"
...
def test_existing_skill_auto_approved_sets_do_not_include_advisor_handoff_save() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    artifact_save = "coach_advisor_handoff_readiness_artifact_save"
...
def test_advisor_handoff_readiness_is_valid_non_activatable_skill() -> None:
    from finance_cli.gateway import tools

    assert "coach_advisor_handoff_readiness" in tools.VALID_SKILLS
    assert "coach_advisor_handoff_readiness" in tools._NON_ACTIVATABLE_SKILLS
...
def test_advisor_handoff_readiness_runtime_auto_approves_only_own_routine_tools(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    runtime = asyncio.run(
        build_runtime(
            SimpleNamespace(approved_tool_types=set(), session_id="sess-handoff"),
            SimpleNamespace(
                model=None,
                context={
                    "user_id": "1",
