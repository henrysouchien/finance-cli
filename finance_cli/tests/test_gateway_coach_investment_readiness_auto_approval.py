...
def test_coach_investment_readiness_auto_approved_set_is_routine_only() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    assert tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED == frozenset(
        {
            "agent_session_write",
    ...
    assert (
        ...
        not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED
    )
    assert "goal_set" not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED
    assert "budget_set" not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED
    assert "notify_schedule" not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED
    assert "setup_monthly_transfer_goal" not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED
    assert "set_monthly_retirement_target" not in tools.COACH_INVESTMENT_READINESS_AUTO_APPROVED


def test_other_skill_auto_approved_sets_do_not_include_investment_save() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    artifact_save = "coach_investment_readiness_artifact_save"
    ...
    assert artifact_save not in tools.COACH_RETIREMENT_CONTRIBUTION_READINESS_AUTO_APPROVED
    assert artifact_save not in tools.COACH_ESTATE_DOCUMENT_READINESS_AUTO_APPROVED


def test_investment_read_tool_is_read_only_and_save_requires_approval() -> None:
    from finance_cli import mcp_server as _mcp_server  # noqa: F401
    from finance_cli.gateway import tools

    assert "coach_investment_readiness_artifact_read" in tools.READ_ONLY_TOOLS
    assert "coach_investment_readiness_artifact_save" in tools.APPROVAL_REQUIRED_TOOLS


def test_investment_readiness_is_valid_non_activatable_skill() -> None:
    from finance_cli.gateway import tools

    assert "coach_investment_readiness" in tools.VALID_SKILLS
    assert "coach_investment_readiness" in tools._NON_ACTIVATABLE_SKILLS
...
def test_investment_readiness_runtime_auto_approves_only_own_routine_tools(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    runtime = asyncio.run(
        build_runtime(
            SimpleNamespace(approved_tool_types=set(), session_id="sess-invest"),
            SimpleNamespace(
                model=None,
                context={
                    "user_id": "1",
